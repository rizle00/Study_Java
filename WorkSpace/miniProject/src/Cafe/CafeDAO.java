...
public class CafeDAO {
	...
	public MenuDTO [] selectRead() { //  함수를 불러야 생성
		...
		cMenus[0] = new MenuDTO(1, "americano", 2000);
		cMenus[1] = new MenuDTO(2, "capuccino", 3000);
		cMenus[2] = new MenuDTO(3, "cafelatte", 3500);
		return cMenus;
	}
	...
	public void displayC(MenuDTO[] cMenus) {
		for (int i=0; i<cMenus.length; i++) {
			System.out.println(cMenus[i].getNum()+"번 메뉴 :"+cMenus[i].getName()+
					", "+cMenus[i].getPrice()+"원");
		}
		
	}
	public void displayM(InfoDTO menu) {
		System.out.print(menu.num+"번 :"+menu.name+" ");
	}
	...
	public MenuDTO [] addMenu(MenuDTO[] cMenus) {
		MenuDTO[] addMenus = new MenuDTO [cMenus.length+1];
		int idx = 0;
		...
		addMenus[addNum-1] = new MenuDTO(addNum, addName, addPrice);
		cMenus = addMenus;
		System.out.println("현재 메뉴 입니다");
		displayC(cMenus);
		return cMenus;
	}
	public void modifyMenu(MenuDTO[] cMenus) {
		
		while(true) {
		System.out.println("수정하실 메뉴를 고르세요 1~"+cMenus.length);
		System.out.println("0입력시 종료합니다");
		int selMenu = select()-1;
		if (selMenu==-1) {
			break;
		}
		...
		if(sel==2) { System.out.println("수정하실 가격을 입력하세요");
			...
			continue;}
		if(sel==3) { System.out.println((selMenu+1)+"번 메뉴 수정 완료");
			break;}
		} 
	}
	}//메뉴수정
	public void deleteMenu(MenuDTO[] cMenus) {
